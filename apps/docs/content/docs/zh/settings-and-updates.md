---
title: 设置、配置迁移与更新
description: 配置监听端口、导入或导出配置文件、同步设置，并接收来自 GitHub Releases 的签名更新。
navTitle: 设置与更新
order: 32
---
...
在 **Settings** 中可以设置本机代理端口、HTTP/2、实验性 QUIC/HTTP/3，以及 DevTools 服务端口。如果当前安装包不支持 QUIC，对应开关会被禁用，并显示具体原因。
...
PostGate 配置文件（Profile）可以包含规则、Values、Replay 集合、证书材料、应用偏好和同步设置。导入时，PostGate 会先读取并展示内容摘要，再恢复其中的数据。你也可以只把兼容的 Whistle 规则导入一个新的规则组，而不恢复完整配置。

包含 CA 私钥、WebDAV 密码、请求头或请求正文的配置文件属于敏感文件。保存或传输时，请像保护凭证一样妥善处理。

## 设置同步

同步与手动迁移使用相同的配置快照格式：

- iCloud 会在支持的 macOS 构建上将配置快照写入 iCloud Drive。
- WebDAV 会把 JSON 快照上传到指定的服务地址和远端路径。

执行 **Push** 或 **Pull** 前，请先保存同步服务设置。Push 会用当前本地状态覆盖远端快照；Pull 会把远端快照导入 PostGate。如果当前环境已经有数据，建议在首次 Pull 前先导出备份。
