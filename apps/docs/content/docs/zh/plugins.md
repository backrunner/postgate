---
title: 插件
description: 构建、安装、启用并调用 PostGate 沙箱 JavaScript 插件。
navTitle: 插件
order: 31
---
...
插件不在 Node.js 中运行，也无法直接访问文件系统、进程或网络等全局 API。所有运行时依赖都必须打包进 JavaScript 入口文件。每次处理函数最多运行五秒，避免异常插件长时间阻塞代理流量。
...
入口文件必须位于包目录内。PostGate 同时支持 CommonJS 和 ESM 默认导出；对于发布包，打包后的 CommonJS 通常最稳定。
...
查询参数会写入 `context.ruleConfig`，`context.matchedPattern` 则包含命中的请求 URL。`handleRequest` 返回响应时会跳过上游请求，返回 `null` 时继续转发；`handleResponse` 可以修改上游响应。
...
- `context.config`：由 PostGate 保存并传入插件的配置。
