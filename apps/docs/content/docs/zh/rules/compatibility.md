---
title: Whistle 兼容性
description: 了解 PostGate 对 Whistle 规则协议的支持范围和行为差异。
navTitle: 兼容性
order: 17
---
...
PostGate 的兼容基线是 Whistle v2.10.6，对应提交 `5e9ac58c979d3713a59fdc3471df296cd0f66071`（2026-07-11）。本文所说的“支持”，是指 PostGate 不仅能够解析规则，还能在 HTTP/1.1、HTTPS 中间人代理和 HTTP/2 流量中实际执行规则。
...
- 资源：代码围栏形式的内联值、全局 Values、本地文件、外部规则和远端 HTTP(S) 正文资源。

## 部分支持或 PostGate 特有

- `xhost` 当前与 `host` 行为相同，尚未实现 Whistle 在连接失败后的回退语义。
- `delete` 可以删除请求头或响应头，但尚未覆盖 Whistle 的所有正文属性、Cookie 和尾部字段删除形式。
...
macOS 发布构建会启用可选的 QUIC 功能，并提供与现有规则链路共用处理流程的本机 HTTP/3 监听入口。该入口不是 MASQUE 代理：PostGate 无法解密或改写 HTTP/3 `CONNECT`、`CONNECT-UDP` 或任意端到端 QUIC 数据报，因此这些请求会返回 `501 Not Implemented`。
