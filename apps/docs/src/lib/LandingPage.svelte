<script lang="ts">
  ...
  const messages = {
    en: {
      ...
      description: 'Capture, rewrite, replay, and debug local web traffic with a native Tauri desktop proxy.',
      ...
      workflows: [
        ['Capture', 'Inspect requests, responses, timing phases, and matched rules without leaving the desktop.'],
        ['Rewrite', 'Redirect hosts, replace files, edit bodies and headers, inject code, simulate latency, throttle bandwidth, or return mocks.'],
    ...
    zh: {
      ...
      description: '使用原生 Tauri 桌面代理捕获、改写、重放和调试本地 Web 流量。',
      ...
      workflows: [
        ['捕获', '在桌面应用中检查请求、响应、耗时阶段和命中的规则。'],
        ['改写', '切换上游、替换文件、修改正文和请求头，或注入代码、模拟延迟与限速。'],
